...
public class SaleRepository implements Repository<Sale> {
    ...
    public void insert(Sale data) {
        String QUERY = "REPLACE INTO sale_table (id, product_id, stock_id, customer_id, quantity, " +
                "purchase_price, sale_price, delivery_price, amazon_code, amazon_request_number, amazon_tax, " +
                "amazon_profit, amazon_SKU, resale_profit, total_profit, date_of_sale, date_of_payment, " +
        ...
        try {
            ...
            if (data.getId() == 0L) {
                ...
                preparedStatement.setInt(10, data.getAmazonTax());
                preparedStatement.setFloat(11, data.getAmazonProfit());
                preparedStatement.setString(12, data.getAmazonSKU());
                preparedStatement.setFloat(13, data.getResaleProfit());
                preparedStatement.setFloat(14, data.getTotalProfit());
                preparedStatement.setLong(15, data.getDateOfSale());
                preparedStatement.setLong(16, data.getDateOfPayment());
                preparedStatement.setLong(17, data.getShippingDate());
                preparedStatement.setLong(18, data.getDeliveryDate());
                preparedStatement.setBoolean(19, data.getIsOrderedByCustomer());
                preparedStatement.setBoolean(20, data.getIsPaidByCustomer());
                preparedStatement.setBoolean(21, data.getDelivered());
                preparedStatement.setBoolean(22, data.getCanceled());
                preparedStatement.setBoolean(23, data.getIsAmazon());
                preparedStatement.setString(24, data.getTimestamp());
            } else {
    ...
    public void update(Sale data) {
        String QUERY = "UPDATE sale_table SET product_id = ?, customer_id = ?, stock_id = ?, quantity = ?, " +
                ...
                "amazon_tax = ?, amazon_profit = ?, amazon_SKU = ?, resale_profit = ?, total_profit = ?, " +
        ...
        try {
            ...
            preparedStatement.setBoolean(23, data.getIsAmazon());
            preparedStatement.setString(24, data.getTimestamp());
            preparedStatement.setLong(25, data.getId());
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Sale> getAll() {
        ...
        String QUERY = "SELECT S.id, S.product_id, S.stock_id, S.customer_id, P.name, C.name as customer_name, " +
                ...
                "PC.categories, P.company, S.amazon_code, S.amazon_request_number, S.amazon_tax, S.amazon_profit, " +
                "amazon_SKU, resale_profit, total_profit, S.date_of_sale, S.date_of_payment, S.shipping_date, " +
                "S.delivery_date, S.is_ordered_by_customer, S.is_paid_by_customer, S.delivered, S.canceled, " +
                "S.time_stamp " +
            ...
            while (resultSet.next()) {
                sales.add(
                        new Sale(
                                ...
                                resultSet.getFloat("resale_profit"),
